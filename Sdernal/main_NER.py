...
from rurebus.vocab import Vocab
...
from rurebus.datasets import BertNerDataset
from rurebus.evaluators import BertNerEvaluator
from rurebus.indexers import BertIndexer
from rurebus.models import BertEncoder
from rurebus.modules import NerTagger
from rurebus.readers import BertReader
from rurebus.samplers import WholeSampler, PartialSampler
from rurebus.trainer import NerTrainer
...
test_folder = 'Sdernal/data/test_ner_only'
documents = reader.read_folder('Sdernal/data/train')
...
model = BertEncoder(bert, 22)
module = NerTagger(indexer.ner_vocab, model)
model.to(device)

results_folder = 'Sdernal/results/NER_BERT'
...
dev_dataset = BertNerDataset(indexer, dev_docs, device)
...
for i in range(5):
    ...
    train_dataset = BertNerDataset(indexer, documents, device)
    train_sampler = RandomSampler(train_dataset)
    train_iterator = DataLoader(train_dataset, batch_size=12, sampler=train_sampler,
                                collate_fn=train_dataset.collate_fn)
    
    trainer = NerTrainer(module, train_iterator, dev_iterator, writer, results_folder)
...
# load best epoch
model.load_state_dict(torch.load(join(results_folder, 'best_ner.pt')))

# evaluate on test
test_dataset = BertNerDataset(indexer, test_docs, device)
test_sampler = SequentialSampler(test_dataset)
test_iterator = DataLoader(test_dataset, batch_size=12, sampler=test_sampler, collate_fn=test_dataset.collate_fn)

# save results
ner_dir = join(results_folder, 'ner')
result = trainer.decode(test_iterator)
evaluator = BertNerEvaluator()
evaluator.save_result(test_dataset, result, ner_dir)
